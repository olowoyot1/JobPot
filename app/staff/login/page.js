...
import { staffLoginAction } from '../../../actions/staff';
...
export default function StaffLoginPage() {
  ...
  function handleSubmit(formData) {
    ...
    startTransition(async () => {
      ...
      else { router.push('/staff'); router.refresh(); }
    });
  }
  ...
  return (
    <main className="max-w-md mx-auto px-6 py-16">
      <div className="bg-white border border-line rounded p-8">
        <h1 className="font-display text-2xl text-navy mb-1">Staff sign in</h1>
        <p className="text-sm text-slate mb-6">Review orders and applicant documents.</p>
        <form action={handleSubmit} className="flex flex-col gap-4">
          ...
          <p className="text-center text-xs text-slate">Staff accounts are created by an admin.</p>
        </form>
      </div>
    </main>
  );
