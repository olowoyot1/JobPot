...
import { affiliateLoginAction } from '../../../actions/affiliates';
...
export default function AffiliateLoginPage() {
  ...
  function handleSubmit(formData) {
    ...
    startTransition(async () => {
      const res = await affiliateLoginAction(formData);
  ...
  return (
    <main className="max-w-md mx-auto px-6 py-16">
      <div className="bg-white border border-line rounded p-8">
        <h1 className="font-display text-2xl text-navy mb-1">Partner sign in</h1>
        <p className="text-sm text-slate mb-6">Track your referrals and commissions.</p>
        <form action={handleSubmit} className="flex flex-col gap-4">
          ...
          <p className="text-center text-sm text-slate">
            New partner? <Link href="/affiliate/signup" className="text-golddark font-semibold underline">Create an account</Link>
          </p>
        </form>
      </div>
    </main>
